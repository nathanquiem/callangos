...
function ApiPage({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  async function revokeToken(token: ApiToken) {
    if (!window.confirm(`Apagar a chave “${token.name}”? Ela deixará de funcionar imediatamente.`)) return
    setRevokingTokenId(token.id)
    try {
      await apiRequest(`/api/v1/api-tokens/${token.id}`, { method: 'DELETE' })
      setTokens((current) => current.filter((item) => item.id !== token.id))
      onNotify('Chave de API apagada.')
    } catch (error) {
      onNotify(error instanceof Error ? `Não foi possível apagar a chave: ${error.message}` : 'Não foi possível apagar a chave.')
    } finally {
